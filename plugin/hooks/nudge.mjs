#!/usr/bin/env node
// SessionStart hook for Shotgun's opt-in "tweetable?" nudge.
//
// Approach (PLAN §3, option A): inject an instruction into the session context
// — exactly how an output style is delivered — so Claude judges tweetability
...
//
// Opt-in and OFF by default: only fires when SHOTGUN_NUDGE is truthy. Annoying
// = uninstalled, so silence is the default.
import { readFileSync } from "node:fs";
import { join } from "node:path";

const flag = (process.env.SHOTGUN_NUDGE ?? "").trim().toLowerCase();
