...
const FULL_ENV = {
  SHOTGUN_X_API_KEY: "ak",
  SHOTGUN_X_API_SECRET: "as",
  SHOTGUN_X_ACCESS_TOKEN: "at",
  SHOTGUN_X_ACCESS_SECRET: "ats",
};
...
test("partial credentials -> still dry-run, reports which are missing", () => {
  const env = { SHOTGUN_X_API_KEY: "ak", SHOTGUN_X_API_SECRET: "as" };
  const config = loadConfig(env);
  assert.equal(config.dryRun, true);
  assert.equal(config.credentials, undefined);
  assert.deepEqual(missingCredentialKeys(env), [
    "SHOTGUN_X_ACCESS_TOKEN",
    "SHOTGUN_X_ACCESS_SECRET",
  ]);
});

test("blank-string credential counts as missing", () => {
  const env = { ...FULL_ENV, SHOTGUN_X_ACCESS_SECRET: "   " };
  assert.deepEqual(missingCredentialKeys(env), ["SHOTGUN_X_ACCESS_SECRET"]);
...
test("full credentials + explicit DRY_RUN -> dry-run honored", () => {
  for (const truthy of ["1", "true", "yes", "on"]) {
    assert.equal(
      loadConfig({ ...FULL_ENV, SHOTGUN_DRY_RUN: truthy }).dryRun,
      true,
  ...
  for (const falsy of ["0", "false", "no", "off"]) {
    assert.equal(
      loadConfig({ ...FULL_ENV, SHOTGUN_DRY_RUN: falsy }).dryRun,
      false,
